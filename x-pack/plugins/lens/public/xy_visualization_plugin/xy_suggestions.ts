...
import { SuggestionRequest, VisualizationSuggestion, TableColumn } from '../types';
...
export function getSuggestions(
  ...
  return Object.entries(opts.tableColumns)
    .filter(
      ([_id, columns]) =>
        // We require a column for x and one for y, and y needs numeric columns...
        columns.length > 1 && columns.find(x => x.operation.dataType === 'number')
    )
    .map(([id, columns]) => getSuggestionForColumns(id, columns));
}
...
function getSuggestionForColumns(
  ...
  const [buckets, values] = partition(prioritizeColumns(columns), x => x.operation.isBucketed);

  if (buckets.length >= 1) {
    const [x, splitBy] = buckets;
    return getSuggestion(id, x, values, splitBy);
  } else {
    const [x, ...yValues] = values;
    return getSuggestion(id, x, yValues);
  }
}
...
function getSuggestion(
  ...
  const yTitle = yValues.map(v => v.operation.label).join(' & ');
  ...
  return {
    ...
    state: {
      ...
      y: {
        accessors: yValues.map(v => v.columnId),
